...
{
    std::string image_file = "/home/jongrok/SLAM/slambook_my/4.Cameras_and_Images/imageBasics/distorted.png";
    ...
    cv::Mat image_undistort = cv::Mat(rows, cols, CV_8UC1);    // the distorted image
    ...
    for (size_t v = 0; v < rows; ++v) {
        for (size_t u = 0; u < cols; ++u) {
            ...
            int u_distorted = static_cast<int>(fx*x_distorted + cx),
                v_distorted =  static_cast<int>(fy*y_distorted + cy);
