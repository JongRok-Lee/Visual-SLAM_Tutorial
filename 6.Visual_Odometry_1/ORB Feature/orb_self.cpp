...
void BFMatch(const std::vector<DescType> &desc1, const std::vector<DescType> &desc2, std::vector<cv::DMatch> &matches, const int &d_max);
...
int main(int argc, char **argv) {
    if (argc != 4) {
        throw std::invalid_argument("usage: feature_extraction img1 img2 distance_max");
    }
    ...
    BFMatch(descriptor1, descriptor2, Matches, std::stoi(argv[3]));
...
void ComputeORB(const cv::Mat &img, std::vector<cv::KeyPoint> &keypoints, std::vector<DescType> &descriptors) {
    ...
    for (auto &kp : keypoints) {
        // Check the FAST patch boundary (8 < y <= rows - 8, 8 < x <= cols - 8)
        if(kp.pt.x < half_boundary || kp.pt.x >= cols - half_boundary ||
           kp.pt.y < half_boundary || kp.pt.y >= rows - half_boundary) {
            // Outside
            ++bad_points;
        ...
        // The moment of the image block
        float m01 = 0, m10 = 0;
        for (int dy = -half_patch_size; dy < half_patch_size; ++dy) {
            for (int dx = -half_patch_size; dx < half_patch_size; ++dx) {
                uint8_t pixel = img.at<uint8_t>(kp.pt.y + dy, kp.pt.x + dx);
                m10 += dx * pixel;
                m01 += dy * pixel;
            }
        }
        
        // Calculate the centroid of the image block (m00 = sqrt(m01^2 + m10^2))
        // cos(theta) = m10/m00, sin(theta) = m01/m00, tan(theta) = m01/m10
        float m00 = std::sqrt(m01 * m01 + m10 * m10) + 1e-18,
              cos_theta = m10 / m00,
              sin_theta = m01 / m00;
        
        // Compute the angle of keypoints
        DescType desc(8, 0);
        for (int i = 0; i < 8; ++i) {
            uint32_t d = 0;
            for (int k = 0; k < 32; ++k) {
                int idx_pq = 32 * i + k;
                cv::Point2f p(ORB_pattern[4 * idx_pq], ORB_pattern[4 * idx_pq + 1]);
                cv::Point2f q(ORB_pattern[4 * idx_pq + 2], ORB_pattern[4 * idx_pq + 3]);
                ...
                // Compare pp and qq
                if (img.at<uint8_t>(pp.y, pp.x) < img.at<uint8_t>(qq.y, qq.x)) {
                    d |= 1 << k;        // | : OR, << : left shift
                }
            }
            desc[i] = d;
...
void BFMatch(const std::vector<DescType> &desc1, const std::vector<DescType> &desc2, std::vector<cv::DMatch> &matches, const int &d_max) {
    ...
    for (int i1 = 0; i1 < desc1.size(); ++i1) {
        ...
        for (int i2 = 0; i2 < desc2.size(); ++i2) {
            ...
            int distance = 0;
            for (int k = 0; k < 8; ++k) {
                distance += _mm_popcnt_u32(desc1[i1][k] ^ desc2[i2][k]);    // ^ : XOR
            }
